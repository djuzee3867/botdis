    ...
    def create_embed(self):
        embed = (discord.Embed(
            ...
            color=discord.Color.blurple()).add_field(
                name='ความยาว', value=self.source.duration).add_field(
                    name='สั่งเพลงโดย',
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send(
                    'ชื่อเพลงผิดอ่ะป่าว ถ้าไม่หายฟาดหัวคนทำบอทเลย : {}'.format(
                        str(e)))
            else:
                song = Song(source)

                await ctx.voice_state.songs.put(song)
                await ctx.send('Playing  {}'.format(str(source)))

    @commands.command(name='meet')
    async def announce(ctx, channel: discord.TextChannel):
        await channel.send(
            "googlemeet \n ภาษาไทย : https://meet.google.com/sje-amzh-wqy?authuser=0&hs=179 \nภาษาอังกฤษ : https://meet.google.com/nzh-qxhf-mbd?authuser=0&hs=179 \nประวัติ : https://meet.google.com/txc-zzyk-tmw?authuser=0&hs=179 \nภาษาอังกฤษเพิ่มวันพฤหัสบดี : http://meet.google.com/fjt-tuvw-qfd?authuser=0&hs=179"
        )
...
bot = commands.Bot(description='น้อนใช้ : นะจ๊ะ', command_prefix=':')
